...
class Context {
  ...
  constructor({
    ...
    // custom webpack
    const webpackPath = this.userConfig.customWebpack
  ...
  public hasRegistration = (
    ...
    const mappedType =
      type === 'cliOption' ? 'cliOptionRegistration' : 'userConfigRegistration';
      ...
      if (!['plugins', 'customWebpack'].includes(configInfoKey)) {
        ...
        if (validation) {
          ...
          if (_.isString(validation)) {
            ...
            assert(
              _[VALIDATION_MAP[validation as ValidationKey]](configValue),
  ...
  private runCliOption = async (): Promise<void> => {
    for (const cliOpt in this.commandArgs) {
      ...
      if (this.command !== 'test' || cliOpt !== 'jestArgv') {
        const { commands, name, configWebpack } =
